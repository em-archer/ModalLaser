...
for n in range(0, 10):
    for m in range(0, 10):
        ...
        ax[0].imshow(np.abs(HGfield)**2,aspect='equal',extent=extent,cmap='cubehelix_r')
        ax[0].set_xlabel(r'x ($\mu m$)')
        ax[0].set_ylabel(r'y ($\mu m$)')
        ax[0].set_title('Intensity')
        ax[1].imshow(np.unwrap(np.angle(HGfield)/(2.*np.pi)),aspect='equal')
        ...
        extent=(x.min()*1e6,x.max()*1e6,y.min()*1e6,y.max()*1e6)
        fig,ax = plt.subplots(1,2)
        ax[0].imshow(np.abs(LGfield)**2,aspect='equal',extent=extent,cmap='cubehelix_r')
        ax[0].set_xlabel(r'x ($\mu m$)')
        ax[0].set_ylabel(r'y ($\mu m$)')
        ax[0].set_title('Intensity')
        ax[1].imshow(np.unwrap(np.angle(LGfield)/(2.*np.pi)),aspect='equal')
        ax[1].set_title('Phase')
